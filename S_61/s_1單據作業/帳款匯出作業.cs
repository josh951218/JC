...
{
    public partial class 帳款匯出作業 : FormT
    {
        ...
        {
            InitializeComponent();
            Txt = new List<TextBox> { ReNo, CoNo, CoName1, CoName2, ReDate, AcNo, AcName, AcName1, Xa1Name, Xa1Par , ReMny, ReMemo, FaNo, FaName1, FaName2, ChargeMny };
            ...
            ChargeMny.NumThousands = ReMny.NumThousands = true;
            ChargeMny.NumLast = ReMny.NumLast = Common.金額小數;
            ChargeMny.NumFirst = ReMny.NumFirst = (20 - 1 - Common.金額小數);
        }
        ...
        void loadM()
        {
            try
            {
                using (SqlConnection cn = new SqlConnection(Common.sqlConnString))
                {
                    ...
                    tbM.Clear();
                    list.Clear();
                    SqlDataAdapter dd = new SqlDataAdapter("select * from remito order by reno", cn);
                    dd.Fill(tbM);
        ...
        private void btnModify_Click(object sender, EventArgs e)
        {
            ...
            DataRow ac = Common.load("Check", "remito", "reno", ReNo.Text.Trim());
            ...
            else
            {
                ...
                try
                {
                    using (SqlConnection cn = new SqlConnection(Common.sqlConnString))
                    {
                        ...
                        cmd.CommandText = "update acct set acmny1+=" + (temp["ReMny"].ToDecimal() + temp["ChargeMny"].ToDecimal()).ToDecimal() + " where acno=N'" + temp["AcNo"].ToString().Trim() + "';";

                        cmd.CommandText += "delete from remito where ReNo=N'"
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            using (帳款匯出作業_列印 frm = new 帳款匯出作業_列印())
            {
                frm.SetParaeter();
                frm.ShowDialog();
            }
        ...
        private void btnBrow_Click(object sender, EventArgs e)
        {
            ...
            using (帳款匯出作業_瀏覽 frm = new 帳款匯出作業_瀏覽())
            {
                frm.SetParaeter();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (btnState == "Append" || btnState == "Duplicate")
            {
                ...
                using (SqlConnection cn = new SqlConnection(Common.sqlConnString))
                {
                    ...
                    try
                    {
                        cmd.CommandText = "insert into remito (reno,cono,coname1,coname2,fano,faname1,faname2,redate,redate_1,acno,acname,acname1,remny,rememo,xa1name,ChargeMny,SurCharge)values("
                                        + " N'" + ReNo.Text.Trim() + "',"
                                        + " N'" + CoNo.Text.Trim() + "',"
                                        + " N'" + CoName1.Text.Trim() + "',"
                                        + " N'" + CoName2.Text.Trim() + "',"
                                        ...
                                        + " N'" + ReMemo.Text.Trim() + "',"
                                        + " N'" + Xa1Name.Text.Trim() + "',"
                                        + " " + ChargeMny.Text.ToDecimal() + ",";
                        if (SurCharge.Checked)
                            cmd.CommandText += "1);";
                        else
                            cmd.CommandText += "0);";
                        cmd.CommandText += "update acct set acmny1-=" + (ReMny.Text.ToDecimal()+ChargeMny.Text.ToDecimal()).ToDecimal() + " where acno=N'" + AcNo.Text.Trim() + "';";
                        cmd.ExecuteNonQuery();
                        tran.Commit();
                        tran.Dispose();
                        cmd.Dispose();
            ...
            if (btnState == "Modify")
            {
                ...
                using (SqlConnection cn = new SqlConnection(Common.sqlConnString))
                {
                    ...
                    try
                    {
                        ...
                        cmd.CommandText = "update acct set acmny1+=" + (temp["ReMny"].ToDecimal() + temp["ChargeMny"].ToDecimal()).ToDecimal() + " where acno=N'" + temp["AcNo"].ToString().Trim() + "'";
                        cmd.ExecuteNonQuery();
                        cmd.CommandText = "delete remito where reno=N'" + CurrentRow + "'";
                        cmd.ExecuteNonQuery();
                        cmd.CommandText = "insert into remito (restno,chstnum,reno,cono,coname1,coname2,fano,faname1,faname2,redate,redate_1,acno,acname,acname1,remny,rememo,xa1name,ChargeMny,SurCharge)values("
        ...
        private void FaNo_Validating(object sender, CancelEventArgs e)
        {
            ...
            if (FaNo.Text.Trim() == "")
            {
                MessageBox.Show("『廠商號碼』不可為空白，請確定後重新輸入", "訊息視窗", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                e.Cancel = true;
                return;
            }
            if (!CHK.FaNo_Validating(Common.浮動連線字串, FaNo, FaName1, FaName2))
        ...
        private void 帳款匯出作業_KeyUp(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.D1:
            ...
            try
            {
                ...
                else
                {
                    using (SqlConnection cn = new SqlConnection(Common.sqlConnString))
                    {
                        cn.Open();
                        SqlCommand cmd = cn.CreateCommand();
                        cmd.CommandText = "select reno from remito where reno='" + ReNo.Text.Trim() + "'";
