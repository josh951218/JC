...
{
    public partial class 帳款匯入作業_列印 : FormT
    {
        ...
        private void 帳款匯入作業_列印_Load(object sender, EventArgs e)
        {
        ...
        RPT paramsInit()
        {
            path = Common.reportaddress + "Report\\帳款匯入作業_標準報表.rpt";
        ...
        void setsql()
        {
            ...
            {
                cn.Open();
                sql = "select * from remiti where redate >= '" + Date.ToTWDate(date1.Text) + "' and redate <= '" + Date.ToTWDate(date2.Text) + "' order by redate";
