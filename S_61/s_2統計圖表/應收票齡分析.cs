...
{
    public partial class 應收票齡分析 : FormT
    {
        List<txtNumber> Rule;
        ...
        private void 應收票齡分析_Load(object sender, EventArgs e)
        {
        ...
        private void btnBrow_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection cn = new SqlConnection(Common.sqlConnString))
                {
                    cn.Open();
                    string sql = "";
                    sql = "select RuleLv1=0.0,RuleLv2=0.0,RuleLv3=0.0,RuleLv4=0.0,RuleLv5=0.0,筆數='',cuno,cuname2,cuname1,chno,chstname,chdate1,chdate1_1,chdate2,chdate2_1,chmny from chki where '0'='0' ";
                ...
                using (SqlConnection cn = new SqlConnection(Common.浮動連線字串))
                {
                    ...
                    {
                        cmd.CommandText = "select cuname1,cuname2 from cust where cuno='" + table.Rows[i]["cuno"].ToString().Trim() + "'";
                        using (reader = cmd.ExecuteReader())
                        {
                            if (reader.HasRows && reader.Read())
                            {
                                table.Rows[i]["cuname1"] = reader["cuname1"].ToString().Trim();
                                table.Rows[i]["cuname2"] = reader["cuname2"].ToString().Trim();
                            }
                        }
                ...
                else
                {
                    ...
                    總額表.Clear();
                    DataRow dr;
                    List<string> cuno_list = table.AsEnumerable().Select(r => r["cuno"].ToString()).Distinct().ToList();
                    DataTable temp;
                    cuno_list.ForEach(r => {
                        decimal lv1 = 0, lv2 = 0, lv3 = 0, lv4 = 0, lv5 = 0;
                        dr = 總額表.NewRow();
                        temp = table.AsEnumerable().Where(t => t["cuno"].ToString().Trim() == r.Trim()).CopyToDataTable();
                        dr["cuno"] = temp.Rows[0]["cuno"].ToString();
                        dr["cuname1"] = temp.Rows[0]["cuname1"].ToString();
