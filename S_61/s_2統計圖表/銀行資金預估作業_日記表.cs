...
{
    public partial class 銀行資金預估作業_日記表 : FormT
    {
        ...
        {
            InitializeComponent();
            this.已收未兌筆數.DefaultCellStyle.Format = "f0";
            this.已付未兌筆數.DefaultCellStyle.Format = "f0";
            this.已收未兌金額.DefaultCellStyle.Format = "N"+Common.金額小數;
            this.已付未兌金額.DefaultCellStyle.Format = "N"+Common.金額小數;
            this.預估餘額.DefaultCellStyle.Format = "N" + Common.金額小數;
        }

        private void 銀行資金預估作業_日記表_Load(object sender, EventArgs e)
        {
        ...
        {
            ViewTb.Clear();
            ViewTb = list.Where(r => r["帳戶編號"].ToString().Trim() == acno).OrderBy(t => t["預兌日"].ToString()).ThenBy(t => t["票態"].ToString()).CopyToDataTable();
            AcNo.Text = ViewTb.Rows[0]["帳戶編號"].ToString().Trim();
            AcName1.Text = ViewTb.Rows[0]["帳戶簡稱"].ToString().Trim();
            已收未兌總額.Text = ViewTb.AsEnumerable().Sum(r => r["已收未兌"].ToDecimal()).ToString("N" + Common.金額小數);
            已付未兌總額.Text = ViewTb.AsEnumerable().Sum(r => r["已付未兌"].ToDecimal()).ToString("N" + Common.金額小數);
            預估總額.Text = ViewTb.Rows[ViewTb.Rows.Count - 1]["預估餘額"].ToDecimal().ToString("N" + Common.金額小數);
        ...
        RPT paramsInit()
        {
            string path = "";
            if (radio1.Checked)
                path = Common.reportaddress + "Report\\銀行資金預估作業_不跳頁日記表.rpt";
            else
                path = Common.reportaddress + "Report\\銀行資金預估作業_跳頁日記表.rpt";
