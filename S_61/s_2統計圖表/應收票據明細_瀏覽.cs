...
{
    public partial class 應收票據明細_瀏覽 : FormT
    {
        ...
        private void 應收票據明細_瀏覽_Load(object sender, EventArgs e)
        {
            ...
            date1.Init();
            date2.Init();
            ChMny.NumThousands = ChOMny1.NumThousands = ChOMny2.NumThousands = ChOMny3.NumThousands = true;
            ChMny.NumLast = ChOMny1.NumLast = ChOMny2.NumLast = ChOMny3.NumLast = Common.金額小數;
            ChMny.NumFirst = ChOMny1.NumFirst = ChOMny2.NumFirst = ChOMny3.NumFirst = (20-1-Common.金額小數);
            this.票面金額.DefaultCellStyle.Format = "N" + Common.金額小數;
            this.差額收現.DefaultCellStyle.Format = "N" + Common.金額小數;
            this.差額付現.DefaultCellStyle.Format = "N" + Common.金額小數;
            this.差額累入預付.DefaultCellStyle.Format = "N" + Common.金額小數;
            if (Common.User_DateTime == 1)
            {
                this.異動日期.DataPropertyName = "chdate";
                this.預兌日.DataPropertyName = "chdate3";
                this.到期日.DataPropertyName = "chdate2";
                this.收票日.DataPropertyName = "chdate1";
            }
            else
            {
                ...
                this.收票日.DataPropertyName = "chdate1_1";
            }
            decimal d = 0, chmny = 0,chomny1 = 0,chomny2 = 0,chomny3 = 0;
            table.AsEnumerable().ToList().ForEach(r =>
            {
                ...
                chomny1 += r["ChOMny1"].ToDecimal();
                chomny2 += r["ChOMny2"].ToDecimal();
                chomny3 += r["ChOMny3"].ToDecimal(); 
            });
            list = table.AsEnumerable().ToList();
            TotCount.Text = table.Rows.Count.ToString();
            ChMny.Text = chmny.ToString("N" + Common.金額小數);
            ChOMny1.Text = chomny1.ToString("N" + Common.金額小數);
            ChOMny2.Text = chomny2.ToString("N" + Common.金額小數);
            ChOMny3.Text = chomny3.ToString("N" + Common.金額小數);
        ...
        private void 應收票據明細_瀏覽_KeyUp(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.F2:
