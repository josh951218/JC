...
namespace S_61.s_2統計圖表
{
    public partial class 應收票齡分析_明細表 : FormT
    {
        ...
        private void 應收票齡分析_明細表_Load(object sender, EventArgs e)
        {
            ...
            cuno_list = table.AsEnumerable().Select(r => r["cuno"].ToString()).Distinct().ToList();
            WriteToTxt(cuno_list.First().ToString().Trim());
        }

        void WriteToTxt(string cuno)
        {
            ViewTb.Clear();
            ViewTb = list.Where(r => r["cuno"].ToString().Trim() == cuno).OrderBy(r => r["chdate1"].ToString()).CopyToDataTable();
            ...
            CuNo.Text = cuno;
            CuName1.Text = list.Find(r => r["cuno"].ToString().Trim() == cuno)["cuname1"].ToString();
            dataGridViewT1.DataSource = ViewTb;
        }

        RPT paramsInit()
        {
            string path = Common.reportaddress + "Report\\應收票齡分析_明細表.rpt";
        ...
        {
            WriteToTxt(cuno_list.First().Trim());
        }

        private void btnPrior_Click(object sender, EventArgs e)
        {
            int index = cuno_list.FindIndex(r => r.ToString().Trim() == CuNo.Text.Trim());
            if (index - 1 < 0)
            {
            ...
            {
                WriteToTxt(cuno_list[index - 1].ToString().Trim());
            }
        }
        ...
        {
            WriteToTxt(cuno_list.Last().Trim());
        }
