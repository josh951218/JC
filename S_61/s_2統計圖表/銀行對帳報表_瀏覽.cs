...
{
    public partial class 銀行對帳報表_瀏覽 : FormT
    {
        ...
        public 銀行對帳報表_瀏覽()
        {
            ...
            this.存入金額.DefaultCellStyle.Format = "N" + Common.金額小數;
            this.領出金額.DefaultCellStyle.Format = "N" + Common.金額小數;
            this.餘額.DefaultCellStyle.Format = "N" + Common.金額小數;
            this.計算值.DefaultCellStyle.Format = "N" + Common.金額小數;
        }

        private void 銀行對帳報表_瀏覽_Load(object sender, EventArgs e)
        {
        ...
        {
            ViewTb.Clear();
            ViewTb = list.Where(r => r["帳戶編號"].ToString().Trim() == acno).OrderBy(t => t["交易日期"].ToString()).ThenBy(t => t["交易狀況"].ToString()).ThenBy(t => t["備註"].ToString()).CopyToDataTable();
        ...
        RPT paramsInit()
        {
            string path = "";
            if(radio1.Checked)
                path = Common.reportaddress + "Report\\銀行對帳報表_不跳頁報表.rpt";
            else
                path = Common.reportaddress + "Report\\銀行對帳報表_跳頁報表.rpt";
