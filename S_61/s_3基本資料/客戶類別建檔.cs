...
{
    public partial class 客戶類別建檔 : FormT
    {
        ...
        public 客戶類別建檔()
        {
            ...
            Txt = new List<TextBox> { X1No,X1Name };
        }

        private void 客戶類別建檔_Load(object sender, EventArgs e)
        {
        ...
        DataRow GetDataRow()
        {
            return list.Find(r => r["X1No"].ToString() == X1No.Text.Trim());
        }

        DataRow GetDataRow(string str)
        {
            return list.Find(r => r["X1No"].ToString() == str.Trim());
        }
        ...
        void WriteToTxt(DataRow dr)
        {
            ...
            else
            {
                ...
                CurrentRow = dr["X1No"].ToString().Trim();
            }
        }
            ...
            else
            {
                ...
                try
                {
                    using (SqlConnection cn = new SqlConnection(Common.浮動連線字串))
                    {
                        ...
                        cmd.CommandText = "delete from xx01 where X1No=N'"
                            + X1No.Text.Trim() + "' COLLATE Chinese_Taiwan_Stroke_BIN";
                        cmd.ExecuteNonQuery();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (btnState == "Append" || btnState == "Duplicate")
            {
                using (SqlConnection cn = new SqlConnection(Common.浮動連線字串))
                {
                    ...
                    try
                    {
                        cmd.CommandText = "insert into xx01 (";
                        Txt.ForEach(r =>
                        {
                            if (r.Name.ToString() != "X1Name")
            ...
            if (btnState == "Modify")
            {
                ...
                using (SqlConnection cn = new SqlConnection(Common.浮動連線字串))
                {
                    ...
                    try
                    {
                        cmd.CommandText = "delete xx01 where X1No='" + CurrentRow + "'";
                        cmd.ExecuteNonQuery();
                        cmd.CommandText = "";
        ...
        private void 客戶類別建檔_KeyUp(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.D1:
        ...
        private void X1No_DoubleClick(object sender, EventArgs e)
        {
            if (X1No.ReadOnly != true)
            {
                using (客戶類別建檔_瀏覽 frm = new 客戶類別建檔_瀏覽())
                {
                    ...
                    {
                        case DialogResult.OK:
                            dr = GetDataRow(frm.Result["X1No"].ToString().Trim());
                            WriteToTxt(dr);
                            break;
                    }
                }
        ...
        private void X1No_Enter(object sender, EventArgs e)
        {
            BeforeText = X1No.Text.Trim();
        }
