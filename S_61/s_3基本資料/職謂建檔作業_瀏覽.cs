...
{
    public partial class 職謂建檔作業_瀏覽 : FormT
    {
        ...
        private void 職謂建檔作業_瀏覽_Load(object sender, EventArgs e)
        {
            ...
            if (list.Count > 0)
            {
                dataGridViewT1.Search("職謂編號", SeekNo);
            }
            X6No.Focus();
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection cn = new SqlConnection(Common.浮動連線字串))
            {
                ...
                CurrentRow = dataGridViewT1.SelectedRows[0].Cells["職謂編號"].Value.ToString().Trim();
                try
                {
                    foreach (DataGridViewRow i in dataGridViewT1.Rows)
                    {
                        cmd.CommandText += "update xx06 set x6name='" + i.Cells["職謂名稱"].Value.ToString().Trim() + "'"
                            + " where x6no='" + i.Cells["職謂編號"].Value.ToString().Trim() + "';";
                    }
                    cmd.ExecuteNonQuery();
                    tran.Commit();
                    loadM();
                    if (list.Count > 0)
                    {
                        dataGridViewT1.Search("職謂編號", CurrentRow);
                    }
                }
                catch (Exception ex)
                {
        ...
        private void btnExit_Click(object sender, EventArgs e)
        {
            if (dataGridViewT1.SelectedRows.Count != 0)
                result = dataGridViewT1.SelectedRows[0].Cells["職謂編號"].Value.ToString().Trim();
            this.DialogResult = DialogResult.OK;
            this.Dispose();
            this.Close();
        }

        private void 職謂建檔作業_瀏覽_KeyUp(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.F11:
        ...
        private void X6No_KeyUp(object sender, KeyEventArgs e)
        {
            if (dataGridViewT1.Rows.Count == 0 || X6No.Text.Trim() == "") return;
            dataGridViewT1.Search("職謂編號",X6No.Text.Trim());
        }
