...
{
    public partial class 貨幣建檔作業_瀏覽 : FormT
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection cn = new SqlConnection(Common.浮動連線字串))
            {
                ...
                CurrentRow = dataGridViewT1.SelectedRows[0].Cells["幣別編號"].Value.ToString().Trim();
                try
                {
                    foreach (DataGridViewRow i in dataGridViewT1.Rows)
                    {
                        cmd.CommandText += "update xa01 set xa1name='" + i.Cells["幣別名稱"].Value.ToString().Trim() + "'"
                            + " where xa1no='" + i.Cells["幣別編號"].Value.ToString().Trim() + "';";
                    }
                    cmd.ExecuteNonQuery();
                    tran.Commit();
                    loadM();
                    if (list.Count > 0)
                    {
                        dataGridViewT1.Search("幣別編號", CurrentRow);
                    }
                }
                catch (Exception ex)
                {
        ...
        private void btnExit_Click(object sender, EventArgs e)
        {
            if (dataGridViewT1.SelectedRows.Count != 0)
                result = dataGridViewT1.SelectedRows[0].Cells["幣別編號"].Value.ToString().Trim();
