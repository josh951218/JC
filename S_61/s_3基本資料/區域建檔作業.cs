...
{
    public partial class 區域建檔作業 : FormT
    {
        ...
        public 區域建檔作業()
        {
            ...
            Txt = new List<TextBox> { X2No,X2Name };
        }

        private void 區域建檔作業_Load(object sender, EventArgs e)
        {
        ...
        DataRow GetDataRow()
        {
            return list.Find(r => r["X2No"].ToString() == X2No.Text.Trim());
        }

        DataRow GetDataRow(string str)
        {
            return list.Find(r => r["X2No"].ToString() == str.Trim());
        }
        ...
        void WriteToTxt(DataRow dr)
        {
            ...
            else
            {
                ...
                CurrentRow = dr["X2No"].ToString().Trim();
            }
        }
            ...
            else
            {
                ...
                try
                {
                    using (SqlConnection cn = new SqlConnection(Common.浮動連線字串))
                    {
                        ...
                        cmd.CommandText = "delete from xx02 where X2No=N'"
                            + X2No.Text.Trim() + "' COLLATE Chinese_Taiwan_Stroke_BIN";
                        cmd.ExecuteNonQuery();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (X2No.Text.Trim() == "")
            {
                MessageBox.Show("『區域編號』不可為空白，請確定後重新輸入", "訊息視窗", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                X2No.Focus();
                return;
            }
            if (btnState == "Append" || btnState == "Duplicate")
            {
                using (SqlConnection cn = new SqlConnection(Common.浮動連線字串))
                {
                    ...
                    try
                    {
                        cmd.CommandText = "insert into xx02 (";
                        Txt.ForEach(r =>
                        {
                            if (r.Name.ToString() != "X2Name")
            ...
            if (btnState == "Modify")
            {
                ...
                using (SqlConnection cn = new SqlConnection(Common.浮動連線字串))
                {
                    ...
                    try
                    {
                        cmd.CommandText = "delete xx02 where X2No='" + CurrentRow + "'";
                        cmd.ExecuteNonQuery();
                        cmd.CommandText = "";
        ...
        private void X2No_Validating(object sender, CancelEventArgs e)
        {
            ...
            if (btnState == "Append")
            {
                ...
                if (i != -1)
                {
                    e.Cancel = true;
                    X2No.Text = "";
                    MessageBox.Show("此區域編號已重複，請重新輸入", "訊息視窗", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            ...
            if (btnState == "Modify")
            {
                ...
                else
                {
                    ...
                    using (區域建檔作業_瀏覽 frm = new 區域建檔作業_瀏覽())
                    {
                        ...
                        {
                            case DialogResult.OK:
                                dr = GetDataRow(frm.Result["X2No"].ToString().Trim());
                                WriteToTxt(dr);
                                break;
                        }
                    }
        ...
        private void X2No_DoubleClick(object sender, EventArgs e)
        {
            if (X2No.ReadOnly != true)
            {
                using (區域建檔作業_瀏覽 frm = new 區域建檔作業_瀏覽())
                {
                    frm.SetParaeter(ViewMode.Normal);
        ...
        private void X2No_Enter(object sender, EventArgs e)
        {
            BeforeText = X2No.Text.Trim();
        }


        private void 區域建檔作業_KeyUp(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.D1:
