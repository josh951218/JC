...
{
    public partial class 職謂建檔作業 : FormT
    {
        ...
        public 職謂建檔作業()
        {
            ...
            Txt = new List<TextBox> { X6No, X6Name };
        }

        private void 職謂建檔作業_Load(object sender, EventArgs e)
        {
        ...
        void loadM()
        {
            try
            {
                using (SqlConnection cn = new SqlConnection(Common.浮動連線字串))
                {
                    ...
                    tbM.Clear();
                    list.Clear();
                    SqlDataAdapter dd = new SqlDataAdapter("select * from xx06 order by x6no", cn);
                    dd.Fill(tbM);
        ...
        DataRow GetDataRow()
        {
            return list.Find(r => r["X6No"].ToString() == X6No.Text.Trim());
        }

        DataRow GetDataRow(string str)
        {
            return list.Find(r => r["X6No"].ToString() == str.Trim());
        }
        ...
        void WriteToTxt(DataRow dr)
        {
            ...
            else
            {
                ...
                CurrentRow = dr["X6No"].ToString().Trim();
            }
        }
            ...
            else
            {
                ...
                try
                {
                    using (SqlConnection cn = new SqlConnection(Common.浮動連線字串))
                    {
                        ...
                        cmd.CommandText = "delete from xx06 where X6No=N'"
                            + X6No.Text.Trim() + "' COLLATE Chinese_Taiwan_Stroke_BIN";
                        cmd.ExecuteNonQuery();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (X6No.Text.Trim() == "")
            {
                MessageBox.Show("『職謂編號』不可為空白，請確定後重新輸入", "訊息視窗", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                X6No.Focus();
                return;
            }
            if (btnState == "Append" || btnState == "Duplicate")
            {
                using (SqlConnection cn = new SqlConnection(Common.浮動連線字串))
                {
                    ...
                    try
                    {
                        cmd.CommandText = "insert into xx06 (";
                        Txt.ForEach(r =>
                        {
                            if (r.Name.ToString() != "X6Name")
            ...
            if (btnState == "Modify")
            {
                ...
                using (SqlConnection cn = new SqlConnection(Common.浮動連線字串))
                {
                    ...
                    try
                    {
                        cmd.CommandText = "delete xx06 where X6No='" + CurrentRow + "'";
                        cmd.ExecuteNonQuery();
                        cmd.CommandText = "";
        ...
        private void 職謂建檔作業_KeyUp(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.D1:
        ...
        private void X6No_Validating(object sender, CancelEventArgs e)
        {
            ...
            if (btnState == "Append")
            {
                ...
                if (i != -1)
                {
                    e.Cancel = true;
                    X6No.Text = "";
                    MessageBox.Show("此職謂編號已重複，請重新輸入", "訊息視窗", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            ...
            if (btnState == "Modify")
            {
                ...
                else
                {
                    ...
                    using (職謂建檔作業_瀏覽 frm = new 職謂建檔作業_瀏覽())
                    {
                        ...
                        {
                            case DialogResult.OK:
                                dr = GetDataRow(frm.Result["X6No"].ToString().Trim());
                                WriteToTxt(dr);
                                break;
                        }
                    }
        ...
        private void X6No_Enter(object sender, EventArgs e)
        {
            BeforeText = X6No.Text.Trim();
        }
    }
}
