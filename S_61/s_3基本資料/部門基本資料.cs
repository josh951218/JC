...
{
    public partial class 部門基本資料 : FormT
    {
        ...
        public 部門基本資料()
        {
            ...
            Txt = new List<TextBox> { DeNo,DeName1 };
        }

        private void 部門基本資料_Load(object sender, EventArgs e)
        {
        ...
        DataRow GetDataRow()
        {
            return list.Find(r => r["DeNo"].ToString() == DeNo.Text.Trim());
        }

        DataRow GetDataRow(string str)
        {
            return list.Find(r => r["DeNo"].ToString() == str.Trim());
        }
        ...
        void WriteToTxt(DataRow dr)
        {
            ...
            else
            {
                ...
                CurrentRow = dr["DeNo"].ToString().Trim();
            }
        }
            ...
            else
            {
                ...
                try
                {
                    using (SqlConnection cn = new SqlConnection(Common.浮動連線字串))
                    {
                        ...
                        cmd.CommandText = "delete from dept where DeNo=N'"
                            + DeNo.Text.Trim() + "' COLLATE Chinese_Taiwan_Stroke_BIN";
                        cmd.ExecuteNonQuery();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (DeNo.Text.Trim() == "")
            {
                MessageBox.Show("『部門編號』不可為空白，請確定後重新輸入", "訊息視窗", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                DeNo.Focus();
                return;
            }
            if (btnState == "Append" || btnState == "Duplicate")
            {
                using (SqlConnection cn = new SqlConnection(Common.浮動連線字串))
                {
                    ...
                    try
                    {
                        cmd.CommandText = "insert into dept (";
                        Txt.ForEach(r =>
                        {
                            if (r.Name.ToString() != "DeName1")
            ...
            if (btnState == "Modify")
            {
                ...
                using (SqlConnection cn = new SqlConnection(Common.浮動連線字串))
                {
                    ...
                    try
                    {
                        cmd.CommandText = "delete dept where DeNo='" + CurrentRow + "'";
                        cmd.ExecuteNonQuery();
                        cmd.CommandText = "";
        ...
        private void 部門基本資料_KeyUp(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.D1:
        ...
        private void DeNo_Validating(object sender, CancelEventArgs e)
        {
            ...
            if (btnState == "Append")
            {
                ...
                if (i != -1)
                {
                    e.Cancel = true;
                    DeNo.Text = "";
                    MessageBox.Show("此部門編號已重複，請重新輸入", "訊息視窗", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            ...
            if (btnState == "Modify")
            {
                ...
                else
                {
                    ...
                    using (部門基本資料_瀏覽 frm = new 部門基本資料_瀏覽())
                    {
                        ...
                        {
                            case DialogResult.OK:
                                dr = GetDataRow(frm.Result["DeNo"].ToString().Trim());
                                WriteToTxt(dr);
                                break;
                        }
                    }
        ...
        private void DeNo_Enter(object sender, EventArgs e)
        {
            BeforeText = DeNo.Text.Trim();
        }
