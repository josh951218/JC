...
{
    public partial class 廠商類別建檔 : FormT
    {
        ...
        public 廠商類別建檔()
        {
            ...
            Txt = new List<TextBox> { X12No, X12Name };
        }

        private void 廠商類別建檔_Load(object sender, EventArgs e)
        {
        ...
        void loadM()
        {
            try
            {
                using (SqlConnection cn = new SqlConnection(Common.浮動連線字串))
                {
                    ...
                    tbM.Clear();
                    list.Clear();
                    SqlDataAdapter dd = new SqlDataAdapter("select * from xx12 order by x12no", cn);
                    dd.Fill(tbM);
        ...
        DataRow GetDataRow()
        {
            return list.Find(r => r["X12No"].ToString() == X12No.Text.Trim());
        }

        DataRow GetDataRow(string str)
        {
            return list.Find(r => r["X12No"].ToString() == str.Trim());
        }
        ...
        void WriteToTxt(DataRow dr)
        {
            ...
            else
            {
                ...
                CurrentRow = dr["X12No"].ToString().Trim();
            }
        }
            ...
            else
            {
                ...
                try
                {
                    using (SqlConnection cn = new SqlConnection(Common.浮動連線字串))
                    {
                        ...
                        cmd.CommandText = "delete from xx12 where X12No=N'"
                            + X12No.Text.Trim() + "' COLLATE Chinese_Taiwan_Stroke_BIN";
                        cmd.ExecuteNonQuery();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (X12No.Text.Trim() == "")
            {
                MessageBox.Show("『類別編號』不可為空白，請確定後重新輸入", "訊息視窗", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                X12No.Focus();
                return;
            }
            if (btnState == "Append" || btnState == "Duplicate")
            {
                using (SqlConnection cn = new SqlConnection(Common.浮動連線字串))
                {
                    ...
                    try
                    {
                        cmd.CommandText = "insert into xx12 (";
                        Txt.ForEach(r =>
                        {
                            if (r.Name.ToString() != "X12Name")
            ...
            if (btnState == "Modify")
            {
                ...
                using (SqlConnection cn = new SqlConnection(Common.浮動連線字串))
                {
                    ...
                    try
                    {
                        cmd.CommandText = "delete xx12 where X12No='" + CurrentRow + "'";
                        cmd.ExecuteNonQuery();
                        cmd.CommandText = "";
        ...
        private void X12No_DoubleClick(object sender, EventArgs e)
        {
            if (X12No.ReadOnly != true)
            {
                using (廠商類別建檔_瀏覽 frm = new 廠商類別建檔_瀏覽())
                {
                    ...
                    {
                        case DialogResult.OK:
                            dr = GetDataRow(frm.Result["X12No"].ToString().Trim());
                            WriteToTxt(dr);
                            break;
                    }
                }
        ...
        private void X12No_Enter(object sender, EventArgs e)
        {
            BeforeText = X12No.Text.Trim();
        }
        ...
        private void X12No_Validating(object sender, CancelEventArgs e)
        {
            ...
            if (btnState == "Append")
            {
                ...
                if (i != -1)
                {
                    e.Cancel = true;
                    X12No.Text = "";
                    MessageBox.Show("此類別編號已重複，請重新輸入", "訊息視窗", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
        ...
        private void 廠商類別建檔_KeyUp(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.D1:
