...
{
    public partial class 貨幣建檔作業 : FormT
    {
        ...
        public 貨幣建檔作業()
        {
            ...
            Txt = new List<TextBox> {  Xa1No, Xa1Name,Xa1Coun1,Xa1Coun2,Xa1Cent };
        }

        private void 貨幣建檔作業_Load(object sender, EventArgs e)
        {
        ...
        DataRow GetDataRow()
        {
            return list.Find(r => r["Xa1No"].ToString() == Xa1No.Text.Trim());
        }

        DataRow GetDataRow(string str)
        {
            return list.Find(r => r["Xa1No"].ToString() == str.Trim());
        }
        ...
        void WriteToTxt(DataRow dr)
        {
            ...
            else
            {
                ...
                CurrentRow = dr["Xa1No"].ToString().Trim();
            }
        }
            ...
            else
            {
                ...
                try
                {
                    using (SqlConnection cn = new SqlConnection(Common.浮動連線字串))
                    {
                        ...
                        cmd.CommandText = "delete from xa01 where Xa1No=N'"
                            + Xa1No.Text.Trim() + "' COLLATE Chinese_Taiwan_Stroke_BIN";
                        cmd.ExecuteNonQuery();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (Xa1No.Text.Trim() == "")
            {
                MessageBox.Show("『幣別編號』不可為空白，請確定後重新輸入", "訊息視窗", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                Xa1No.Focus();
                return;
            }
            if (btnState == "Append" || btnState == "Duplicate")
            {
                using (SqlConnection cn = new SqlConnection(Common.浮動連線字串))
                {
                    ...
                    try
                    {
                        cmd.CommandText = "insert into xa01 (";
                        Txt.ForEach(r =>
                        {
                            if (r.Name.ToString() != "Xa1Cent")
            ...
            if (btnState == "Modify")
            {
                ...
                using (SqlConnection cn = new SqlConnection(Common.浮動連線字串))
                {
                    ...
                    try
                    {
                        cmd.CommandText = "delete xa01 where Xa1No='" + CurrentRow + "'";
                        cmd.ExecuteNonQuery();
                        cmd.CommandText = "";
        ...
        private void 貨幣建檔作業_KeyUp(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.D1:
        ...
        private void Xa1No_Validating(object sender, CancelEventArgs e)
        {
            ...
            if (btnState == "Append")
            {
                ...
                if (i != -1)
                {
                    e.Cancel = true;
                    Xa1No.Text = "";
                    MessageBox.Show("此幣別編號已重複，請重新輸入", "訊息視窗", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            ...
            if (btnState == "Modify")
            {
                ...
                else
                {
                    ...
                    using (貨幣建檔作業_瀏覽 frm = new 貨幣建檔作業_瀏覽())
                    {
                        ...
                        {
                            case DialogResult.OK:
                                dr = GetDataRow(frm.Result["Xa1No"].ToString().Trim());
                                WriteToTxt(dr);
                                break;
                        }
                    }
